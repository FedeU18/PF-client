...
export const ProfeCard=({id,nombre,descripcion,imagen,precio,materias,puntuacion,username,pais})=> {
 ...
 useEffect(()=>{
  ...
  const handleFav=()=>{
    ...
    if(infoAlumno.favourites && infoAlumno.favourites!==null && infoAlumno.favourites.includes(id)){

      dispatch(editAlumno({favourites:infoAlumno.favourites.filter(l=>l!==id)},infoAlumno.id))
      setFav(false)
  }
  ...
  if(!infoAlumno.favourites.includes(id)){
      console.log('lo')
      dispatch(editAlumno({favourites:[...infoAlumno.favourites,id]},infoAlumno.id))
      setFav(true)
  }
   ...
   <Card style={{ width: '18rem' , margin:'16px' }}>
     ...
     <Link to={"/profesores/" + id}>
      ...
      <Card.Body>
        ...
        <AiFillStar size={22} />
         <div className='puntProfCard'>{puntuacion?.length>0 ? promedio/(puntuacion?.length):0}</div>
        </div>
        <hr></hr>
        <div className='precioFavCont'>
          <button  onClick={handleFav} className='btnFavProfeCards'>
            {fav?(
              ...
              <MdOutlineFavoriteBorder  size={26} style={{  cursor:'pointer' }} />
            )}
          </button>
          <div>
            {precio} US$ por hora
